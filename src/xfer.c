/* $Id: xfer.c,v 1.11  2005/01/16 10:49:16  jon Exp $
 *
...
enum {
	FILE_Not_R = 1, FILE_Not_W = 2, FILE_Not_X = 4, FILE_Not_D = 8,
...
static void crccalc(char *buffer, long len, long *crc)
{
...
/* Send a *FREE command to get total disc size */

static bbc_status_t getADFSsize(serial_h com, int *space)
{
	bbc_status_t result;
	char buff1[MAXLINELEN];
	char buff2[MAXLINELEN];
	int free = 0, used = 0;

	serial_printf(com, "*FREE\r");

	if ((result = bbc_readline(com, buff1, MAXLINELEN)) == BBC_OK) {
		if (sscanf(buff1, "%x", &free) != 1) {
			printf("FREE returned does not match expected format: %s\n", buff1);
		}
	}
	else
		printf("FREE did not return expected result\n");

	if ((result = bbc_readline(com, buff2, MAXLINELEN)) == BBC_OK) {
		if (sscanf(buff2, "%x", &used) != 1) {
			printf("FREE returned does not match expected format: %s\n", buff2);
		}
	}
	else
...
/* Retrieve a single sided DFS disc image */
...
static bbc_status_t retrieve_disc(serial_h com)
{
	...
	serial_printf(com, "*DISC\r");
	do {
		result = bbc_readline(com, buffer, MAXLINELEN);
	} while (result == BBC_OK);

	if (result != BBC_SYNC) {
		printf("Problem changing to DFS\n");
		return result;
	}
...
/* Retrieve a double sided DFS disc image */

static bbc_status_t retrieve_dbl_disc(serial_h com)
{
	char drivech, *extn = NULL, buffer[BBCTRACKSIZE], imagename[MAXLINELEN];
	bbc_status_t result;
	FILE *image;
	...
	printf("Drive number (0, 1, S to skip): ");
	do {
		drivech = toupper(keyboard_char());
	} while (strchr("01S", drivech) == NULL);
	...
	for (ntracks = 0; !interrupted(false) && !trackerr && ntracks < size; ++ntracks) {
		...
		serial_printf(com, "G%c\r%d\r", drivech + (ntracks & 1) * 2, (ntracks >> 1));
		do {
			long rbytes = remaining;
...
/* Retrieve an ADFS disc image */

static bbc_status_t retrieve_ADFS_disc(serial_h com, bool force_single_side)
{
	char drivech, *extn = NULL, buffer[BBCADFSTRACKSIZE], imagename[MAXLINELEN];
	bbc_status_t result;
	FILE *image;
	int ntracks = 0;
	bool trackerr = false;
	...
	serial_printf(com, "*FADFS\r");
	do {
		result = bbc_readline(com, buffer, MAXLINELEN);
	} while (result == BBC_OK);

	if (result != BBC_SYNC) {
		printf("Problem changing to ADFS\n");
		return result;
	}

	serial_printf(com, "*MOUNT %c\r", drivech);
	do {
		result = bbc_readline(com, buffer, MAXLINELEN);
	} while (result == BBC_OK);
	...
	/* Get number of sectors */
	if ((result = getADFSsize(com, &size)) != BBC_SYNC) {
		printf("Problem reading number of sectors on drive %c\n", drivech);
		return result;
	}
	...
	case 160 * 16:
		...
		break;
	default:
		printf("Unrecognised disc format, drive %c has %d sectors\n", drivech, size);
	}
	...
	/* Read whole disc, one track at a time */
	for (ntracks = 0; !interrupted(false) && !trackerr && ntracks < size; ++ntracks) {
		long remaining = BBCADFSTRACKSIZE, crc = 0;

		track = ntracks;
		if (size > 80)
		{
			track = (ntracks / 2) + (ntracks % 2) * 80;   // Interleaved double sided disc
		}

		/* Send command to BBC to transfer track over serial port */
		serial_printf(com, "A%c\r%d\r", drivech, track);
		...
		do {
			...
			//        printf("Read %d tracks of %d, Bytes %d, Remaining %d\n",
			//            ntracks + 1, size, rbytes, remaining) ;
...
static bbc_status_t send_file(serial_h com, char *inffile)
{
	...
	bbc_status_t result = BBC_OK;
	long load = 0xffffffffL, exec = 0xffffffffL, length = 0, attr = 0, crc = 0;
	int nscan, type = 1, opt4 = -1;
	char *scan;
	FILE *fileh;
	bool fileerr = false, haslen = false;
	...
	if (scan != NULL &&
		(sscanf(scan, "TAPE %s %lx %lx%n", bbcname,
			&load, &exec, &nscan) == 3 ||
			sscanf(scan, "%s %lx %lx%n", bbcname,
				&load, &exec, &nscan) == 3)) {
		scan += nscan;
		if (sscanf(scan, " %lx%n", &length, &nscan) == 1 &&
			(scan[nscan] == '\0' || isspace(scan[nscan]))) {
			scan += nscan;
			haslen = true;
		}
		nscan = 0;
		...
		do {
			...
			else if (strncmp(scan, "CRC=", 4) == 0) {
				scan += 4;
				while (*scan != '\0' && !isspace(*scan)) ++scan;
			}
			else if (strncmp(scan, "NEXT", 4) == 0) {
				scan += 4;
				while (*scan != '\0' && isspace(*scan)) ++scan;
				while (*scan != '\0' && !isspace(*scan)) ++scan;
			}
	...
	if (haslen) {
		if (ftell(fileh) != length) {
			printf("Length does not match length in INF, skipping %s\n", pcname);
			fclose(fileh);
			return BBC_OK;
		}
	}
	...
	if ((result = bbc_readline(com, buffer, MAXLINELEN)) == BBC_SYNC) {
		...
		while (nbytes < length) {
			long wbytes = remaining > SENDBUFSIZE ? SENDBUFSIZE : remaining;
 ...
 */
 ...
 /* Sending dfs disc image to BBC */

static bbc_status_t send_dfs_disc_image(serial_h com)
{
	char drivech, *extn = NULL, buffer[BBCTRACKSIZE], imagename[MAXLINELEN];
	char line[BBCTRACKSIZE];
	bbc_status_t result;
	FILE *image;
	long size = 0, ntracks = 0, track = 0;
	bool trackerr = false;
	int disktype = -1;
	char tempdrivech;
	long bbccrc = 0;
	...
	extn = strrchr(imagename, '.');
	if (extn) {
		if (strcmp(extn, ".ssd") == 0 || strcmp(extn, ".SSD") == 0)
		{
			disktype = 1;
		}
		if (strcmp(extn, ".dsd") == 0 || strcmp(extn, ".DSD") == 0)
		{
			disktype = 2;
		}
	}
	...
	if ((size % 2560) == 0)   // If whole number of tracks
	{
		size = size / 2560;
	}
	else {
		size = size / 2560 + 1; // else round up to next track
	}

	fseek(image, 0, SEEK_SET);


	/* Read whole disc, one sector at a time */
	...
	for (ntracks = 0; !interrupted(false) && !trackerr && ntracks < size; ntracks++)
	{
		...
		memset(buffer, 0, 2560);    // In case only half a track read in

		fread(buffer, sizeof(char), 2560, image);
		...
		else                  // dsd
		{
			...
			tempdrivech = drivech + head * 2;
		}

		serial_printf(com, "g%c\r%d\r", tempdrivech, track);

		if ((result = bbc_readline(com, line, MAXLINELEN)) == BBC_SYNC)
		{

			switch (serial_write(com, buffer, 2560))
			{
			case SERIAL_ERROR:
				error("Communications failure");
			case SERIAL_TIMEOUT: /* Want more sensible error handling here */
			...
			crccalc(buffer, 2560, &crc);

			printf("\rWritten %ld tracks of %ld", ntracks + 1, size);

			fflush(stdout);

			/* Get checksum */

			if ((result = bbc_readline(com, line, MAXLINELEN)) != BBC_OK ||
				sscanf(line, "%ld", &bbccrc) != 1)
			{
				printf("\nProblem retrieving CRC for disc %c track %ld from BBC\n",
					tempdrivech, track);
				trackerr = true;
			...
			printf("\nReceived no reply for disc %c track %ld from BBC\n",
				tempdrivech, track);
			trackerr = true;
		}
	}
...
/* Sending ADFS disc image to BBC */

static bbc_status_t send_adfs_disc_image(serial_h com)
{
	char drivech, *extn = NULL, buffer[BBCADFSTRACKSIZE], imagename[MAXLINELEN];
	...
	if (extn)
	{
		if (strcmp(extn, ".adf") == 0 || strcmp(extn, ".ADF") == 0)
		{
			disktype = 1;
		}
		if (strcmp(extn, ".adl") == 0 || strcmp(extn, ".ADL") == 0)
		{
			disktype = 2;
		}
	}
	...
	if ((size % BBCADFSTRACKSIZE) == 0)   // If whole number of tracks
	{
		size = size / BBCADFSTRACKSIZE;
	}
	else {
		size = size / BBCADFSTRACKSIZE + 1; // else round up to next track
	}

	fseek(image, 0, SEEK_SET);

	/* Get number of sectors */
	if ((result = getADFSsize(com, &space)) != BBC_SYNC) {
		printf("Problem reading number of sectors on drive %c\n", drivech);
		return result;
	}
	...
	space /= 16;
	if (size > space) {
		printf("Disc %c is too small for the image\n", drivech);
		return BBC_OK;
	}
	...
	for (ntracks = 0; !interrupted(false) && !trackerr && ntracks < size; ntracks++)
	{
		...
		memset(buffer, 0, BBCADFSTRACKSIZE);    // In case only half a track read in

		fread(buffer, sizeof(char), BBCADFSTRACKSIZE, image);
		...
		if ((result = bbc_readline(com, line, MAXLINELEN)) == BBC_SYNC)
		{
			...
			switch (serial_write(com, buffer, BBCADFSTRACKSIZE))
			{
			case SERIAL_ERROR:
				error("Communications failure");
			case SERIAL_TIMEOUT: /* Want more sensible error handling here */
			...
			crccalc(buffer, BBCADFSTRACKSIZE, &crc);
	...
	/*
	 * Remount to re-read new directory
	 */
...
void xfer(serial_h com, configuration_t *config)
{
	...
	do {
		...
		getcwd(curdir, MAXLINELEN);

		printf("\n[log=%s] [%s] Command: ", log ? "on " : "off", curdir);

		do {
			ch = toupper(keyboard_char());
		} while (strchr("RS*DTXGHALCPBQE?/", ch) == NULL);
		...
		case 'L':
			...
			if (log) {
				...
				printf("Log *command output to file: ");
		...
		case 'H':
			putchar('\n');
			result = retrieve_dbl_disc(com);
			break;
		case 'A':
			putchar('\n');
			result = retrieve_ADFS_disc(com, false);
			break;
		case 'E':
			putchar('\n');
			result = retrieve_ADFS_disc(com, true);
			break;
		case 'T':
			putchar('\n');
			result = send_dfs_disc_image(com);
			break;
		case 'X':
			putchar('\n');
			result = send_adfs_disc_image(com);
			break;
		case 'D':
		...
		case '?':
			printf("\n"
				"   (G) Get DFS single-sided disc image (H) Get DFS double-sided disc image\n"
				"   (A) Get ADFS disc image             (R) Get files\n"
				"   (E) Get ADFS single-sided disc image\n"
				"   (T) Send DFS disc image             (X) Send ADFS disc image\n"
				"   (S) Send files                      (*) Do *command\n"
				"   (P) Checksum local files            (B) Checksum remote files\n"
				"   (L) Toggle *command output logging  (C) Change PC directory\n"
				"   (D) Do PC command                   (Q) Quit\n");
			break;
		}
...
/*
 * $Log: xfer.c,v $
 * Revision 1.12  2005/01/22 08:32:05  jon
 * Added support for transferring disc images back from PC to BBC
 *
 * Revision 1.11  2005/01/16 10:49:16  jon
 * Added ADFS support
 *
