...
void DumpHex(BYTE *buffer, int len);
...
static char *GetLastErrorMessage(void)
{
  ...
  if ( dwRet == 0 ) {      /* No error string; what to do? */
    ...
    sprintf(message, "error %d (%x)", (int) err, (unsigned int) err) ;
    return message ;
...
serial_status_t serial_config(serial_h connection, configuration_t *config)
{
  ...
  dcb.fOutxCtsFlow =
          config->handshake == HANDSHAKE_CTS_RTS ? 1 : 0;
                             /* CTS output flow control */
  dcb.fOutxDsrFlow =
          config->handshake == HANDSHAKE_DSR_DTR ? 1 : 0;
  ...
  dcb.fOutX =
          config->handshake == HANDSHAKE_XON_XOFF ? 1 : 0;
  ...
  dcb.fRtsControl =
         config->handshake == HANDSHAKE_CTS_RTS ?
  ...
  if ( !FlushFileBuffers((HANDLE)connection) ||
    ...
    if ( ClearCommError((HANDLE)connection, &errs, &cstat) ) {
      printf("DSR %d CTS %d RLSD %d TXOFF %d XOFF %d EOF %d RXQ %d TXQ %d\n"
             ...
             (int) cstat.cbInQue, (int) cstat.cbOutQue,
             (unsigned int) errs) ;
    }
...
serial_status_t serial_read(serial_h connection, char *buffer, long *size)
{
  ...
  if ( !ReadFile((HANDLE)connection, buffer, (DWORD)*size, &rbytes, NULL)) {
    ...
    printf("Read error %d %s\n", (int) GetLastError(), GetLastErrorMessage()) ;
    if ( ClearCommError((HANDLE)connection, &errs, &cstat) ) {
      printf("DSR %d CTS %d RLSD %d TXOFF %d XOFF %d EOF %d RXQ %d TXQ %d\n"
             "ERRS %x\n"
             "rbytes %d\n",
...
void default_configuration(configuration_t *config, char *home)
{
  ...
  sprintf(ini, "%sxfer.ini", home);

//printf("Reading from %s\n", ini);

#define GETINISTRING(key, value) \
  GetPrivateProfileString("XFer", (key), (value), buffer, MAXLINELEN, ini)
  ...
  while (num > 0)
        {
                ...
                for (i = 0; i < num; ++i)
                {
                        sprintf(p, "%02X ", (BYTE) buffer[addr + i]);
                        p += 3;
                }
